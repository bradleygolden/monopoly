...
public class Game
{
    public static final String[] TOKENS = {"Scottish Terrier", "Battleship", "Automobile", "Top Hat",
        "Thimble", "Boot", "Wheelbarrow", "Cat"};             // the list of tokens
    private final Color SADDLE_BROWN = new Color(150, 75, 0); // sandle brown color
    private final Color SKY_BLUE = new Color(135, 206, 235);  // sky blue color
    private final Color DARK_ORCHARD = new Color(153,50,204); // dark orchard color
    private final Color ORANGE = Color.ORANGE;                // orange color
    private final Color RED = Color.RED;                      // red color
    private final Color YELLOW = Color.YELLOW;                // yellow color
    private final Color GREEN = Color.GREEN;                  // gree color
    private final Color BLUE = Color.BLUE;                    // blue color
    ...
    //POST: the action will be performed and FUNCVAL == true
    {
        ...
        if(currentLocation instanceof Lot ||
           ...
           currentLocation instanceof Utility) // player is on one of these three
        {
            if(action.equals("Purchase")) // player selects Purchase 
            { 
                currentPlayer.removeMoney(((Property) currentLocation).getPrice());
                currentPlayer.addProperty((Property) currentLocation);
            }
            if(action.equals("Pay Rent")) // player selects Pay Rent
            {
                ...
                {
                    if(currentPlayer.getMoney() < ((Property) currentLocation).getRent(currentPlayer)) // player has enough money
                    {
                        return false; // cannot pay
                    }
        ...
        if(currentLocation instanceof CornerSquare) // player lands on corner square
        {
            if(action.equals("Collect Money"))  // player selects collect money
            {
                currentPlayer.addMoney(((CornerSquare) currentLocation).collectMoney());
            }
        }
        if(currentLocation instanceof CardSquare) // player lands on card square
        {
            if(action.equals("Collect Money")) // player selects collect money
            {
        ...
        if(currentLocation instanceof TaxSquare) // player lands on tax square
        {
            if(action.equals("Pay Tax")) // player slecets Pay Tax
            {
                int money = ((TaxSquare) currentLocation).getTax();
                if(currentPlayer.getMoney() < money) // player does not have enough money
                {
        ...
        if(currentLocation instanceof Jail) // player lands on jail square
        {
            if(action.equals("Pay Rent")) // player selects Pay Rent
            {
                int cost = ((Jail) currentLocation).getRent();
                if(currentPlayer.getMoney() >= cost) // player has enough money to pay rent
                {
    ...
    //POST: the game will start by randomizing the order of the players
    {
        ...
        for(int i = 0; i < players.length * 2; i++) // scramble player order
        {
    ...
    {
        if(players[currentPlayerIndex].getBoardLocation() instanceof Lot) // player lands on lot
        {
    ...
    //POST: removes current player from game
    {
        ...
        for(int i = 0, j = 0; i < players.length; i++) // loop though players
        {
            if(players[i] != null)
            {
                newPlayers[j] = players[i]; // add to new player array
                j++;
            }
        }
        players = newPlayers;
    ...
    //POST: returns the array of 28 properties on the board
    {
        ...
        for(int i = 0; i < 28; i++) // loop through to make array of properties
        {
    ...
    // PRE: property is valid
    //POST: returns true if improvement was sold false otherwise
    {
        if(p instanceof Lot) // can only improve lot, so check for this condition
        {
            int cost = ((Lot) p).getImprovementCost() / 2;
            if(((Lot) p).sellImprovement()) // sell improvement went through
            {
    ...
    {
        if(p instanceof Lot) // only lot can be improved
        {
            int cost = ((Lot) p).getImprovementCost();
            if(players[currentPlayerIndex].getMoney() >= cost) // player has the money
            {
                if(((Lot) p).improve()) // lot has been improved
                {
                    players[currentPlayerIndex].removeMoney(cost);
                    return true;
                }
                else // lot cannot be improved
                {
                    return false;
                }
            }
    ...
    //POST: the player will be initialized and added to the list of players for the game
    {
        ...
        if(players == null) // no players currently
        {
    ...
    {
        String s = "";
        for(BoardLocation b : board) // loop through all board locations
        {
            s += b.toString();
        }
        return s;
    ...
    //POST: returns toStrings of the players
    {
        ...
        for(Player p : players) // loop through players
        {
            ...
            if(properties == null) // they own no properties
            {
                s += p.simpleToString() + ", Owns no properties\n";
            }
            else // they have properties
            {
                s += p.simpleToString() + ", Owns properties:\n";
                for(Property prop : properties) // loop through properties
                {
